...
You can always view the finished code on [GitHub](https://github.com/adobe/aem-guides-wknd-spa) or check the code out locally by switching to the branch `React/integrate-spa-solution`.
...
1. In the `ui.frontend` module create a new folder named `Header` beneath `ui.frontend/src/components`.
...
8. Return to the terminal and navigate into the `ui.apps` folder. Execute the command `mvn clean install -PautoInstallPackage`:
...
1. Return to the IDE and open the file `ui.frontend/package.json`. Look for a line like the following:

    ```json
    "proxy": "http://localhost:4502",
    ```

    The [Create React App](https://create-react-app.dev/docs/proxying-api-requests-in-development) provides an easy mechanism to proxy API requests. All unknown requests will be proxied through `localhost:4502`.
...
3. Open a new browser tab (if not already opened) and navigate to [http://localhost:3000/content/wknd-spa-react/us/en/home.html](http://localhost:3000/content/wknd-spa-react/us/en/home.html).
    ...
    >[!NOTE]
    >
    > Due to the security requirements of AEM, you will need to be logged into the AEM instance (http://localhost:4502) in the same browser.
...
# JSON API via statick mock file
REACT_APP_PAGE_MODEL_PATH=/mock-content/mock.model.json
```
...
8. Create a new file named `_Header.scss` beneath `ui.frontend/src/components/Header`. Populate the file with the following:
...
9. Include `_Header.scss` by updating `Header.js`:
...
1. Open a terminal and navigate to the `ui.frontend` directory. Stop the **webpack-dev-server**, if started, and run the following commands to reset the `node` dependencies:
    ...
    This is a one time activity needed since `node-sass` was installed and the [frontend-maven-plugin](https://github.com/eirslett/frontend-maven-plugin) doesn't know about it.
