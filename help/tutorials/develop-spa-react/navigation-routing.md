...
2. Clean and deploy the code base to a local AEM instance using Maven:
...
You can always view the finished code on [GitHub](https://github.com/adobe/aem-guides-wknd-spa) or check the code out locally by switching to the branch `React/navigation-routing-solution`.

## Configure the Header as part of the Template
...
4. Open a browser and login to AEM, [http://localhost:4502/](http://localhost:4502/). The starting code base should already be deployed.
5. Navigate to the **SPA Page Template**: [http://localhost:4502/editor.html/conf/wknd-spa-react/settings/wcm/templates/spa-page-template/structure.html](http://localhost:4502/editor.html/conf/wknd-spa-react/settings/wcm/templates/spa-page-template/structure.html).
6. Select the **Root Layout Container** and click its **Policy** icon:
...
7. Refresh the page, and add the **Header** component above the un-locked **Layout Container**:

    ![add Header component to template](./assets/navigation-routing/add-header-component.gif)

8. Select the **Header** component and click its **Policy** icon to edit the policy.
9. Create a new policy with a **Policy Title** of **WKND SPA Header**.
...
10. After saving your changes you should see the populated `Header` as part of the template:
...
6. In a new tab, open the JSON model API provided by AEM: [http://localhost:4502/content/wknd-spa-react/us/en.model.json](http://localhost:4502/content/wknd-spa-react/us/en.model.json). This JSON content is requested when the SPA is first loaded. The outer structure looks like the following:
    ...
    Of course, it is not wise to load ALL of the content of a SPA in the initial JSON request, since this would slow down the first page load. Next lets look at how the we configure the heirarchy depth of pages collected.
...
9. Re-open the JSON Model [http://localhost:4502/content/wknd-spa-react/us/en.model.json](http://localhost:4502/content/wknd-spa-react/us/en.model.json).
    ...
    You should now see that the **Page 3** path has been removed: `/content/wknd-spa-react/us/en/home/page-2/page-3` from the initial JSON model.

    Later in the chapter, we will see how the AEM SPA Editor SDK can dynamically load additional content, that was not included in the initial JSON model.
...
3. Open a new browser tab and navigate to [http://localhost:3000/](http://localhost:3000/).
    ...
    The SPA project is set up to proxy the JSON model from a local instance of AEM. This will allow us to code directly against the content created in AEM in the previous exercise. Ensure that you are authenticated into AEM in the same browsing session.
...
6. Add a line at the top to import the `Navigation` component beneath the other imports:
...
7. Next update the `get navigation()` method to instantiate the `Navigation` component:
    ...
    As mentioned earlier, instead of implementing the navigation within the `Header` we will implement the majority of the logic in the `Navigation` component. Since the `Header`'s props include the JSON structure needed to build the menu, we pass all of the props.
...
9. Implement the `renderGroupNav(children)` method:
    ...
    This method takes the an array of navigation items, `children`, and creates an un-ordered list. It then iterates over the array and passes the item to the `renderNavItem` which we will implement next.
...
2. Next, deploy the updates to AEM from a terminal, using your Maven skills:
...
3. Navigate to the SPA homepage in AEM: [http://localhost:4502/content/wknd-spa-react/us/en/home.html](http://localhost:4502/content/wknd-spa-react/us/en/home.html) and open your browser's developer tools (screenshots captured from Chrome).
...
## Bonus - Static, non-AEM routes

Many SPA implementations will have some views of the SPA that do **not** map to an AEM Page. These would be 
