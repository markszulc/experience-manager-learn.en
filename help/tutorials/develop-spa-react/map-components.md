...
You can always view the finished code on [GitHub](https://github.com/adobe/aem-guides-wknd-spa) or check the code out locally by switching to the branch `React/map-components-solution`.
...
7. Experiment by modifying the `mock.model.json` file at `ui.frontend/public/mock-content/mock.model.json`. At ~line 62 update the first `Text` value:
...
1. Navigate to the [Image examples in the Core Component library](https://www.aemcomponents.dev/content/core-components-examples/library/page-authoring/image.html).
    ...
    Properties like `src`, `alt`, and `title` that will be used to populate the `Image` component.

    >[!NOTE]
    >
    > There are other Image properties exposed (`lazyEnabled`, `widths`) that would allow a developer to create an adaptive and lazy-loading image. For this tutorial we will keep it simple.
...
10. Return to [http://localhost:3000](http://localhost:3000) and you should see the image render:
...
The `Image` component is only visible in the **webpack-dev-server**. Next, deploy the updated SPA to AEM and update the 
...
2. From the AEM Start screen navigate to **Tools** &gt; **Templates** &gt; **WKND SPA React**. 
...
5. In the **Layout Container** click the **policy** icon for the **Text** component:
    ...
    Under **Properties** &gt; **Paragraph Styles** &gt; check the box to **Enable paragraph styles**:
...
6. Navigate to the **Homepage** [http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html](http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html).
    ...
    You should now be able to drag+drop an image from the **Asset finder**:
...
7. Add your own images via [AEM Assets](http://localhost:4502/assets.html/content/dam) or install the finished code base for the traditional [WKND reference site](https://github.com/adobe/aem-guides-wknd/releases/latest). The [WKND reference site](https://github.com/adobe/aem-guides-wknd/releases/latest) includes many images that will be re-used on the WKND SPA. The package can be installed using [AEM's Package Manager](http://localhost:4502/crx/packmgr/index.jsp).
...
1. In a browser navigate to [http://localhost:4502/content/wknd-spa-react/us/en.model.json](http://localhost:4502/content/wknd-spa-react/us/en.model.json)
    ...
    The **Layout Container** component has a `sling:resourceType` of `wcm/foundation/components/responsivegrid` and is recognized by the SPA Editor using the `:type` property like the `Text` and `Image` component.
...
2. Return to [http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html](http://localhost:4502/editor.html/content/wknd-spa-react/us/en/home.html) and try re-sizing one of the image components using the **Layout** option:
