---
title: Develop a Asset Compute metadata worker
description: Asset Compute workers can be developed to write back metadata instead.
...
# Develop a Asset Compute metadata worker
...
+ Deriving metadata about the asset from the asset binary and storing it as accessible metadata on the asset
...
In this tutorial we'll create an Asset Compute metadata worker that derives the most commonly used colors in an image asset, and writes the names of the colors back to the asset's metadata in AEM.
...
1. Navigate to __Tools > Assets > Metadata Schemas__
...
## Worker metadata-colors/index.js on Github

The final `metadata-colors/index.js` is available on Github at:

+ [aem-guides-wknd-asset-compute/actions/worker/index.js](https://github.com/adobe/aem-guides-wknd-asset-compute/blob/master/actions/metadata-colors/index.js)
